  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="nursingList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="View" @click="handleViewData(scope.row)" v-hasPermi="['system:nursing:query']">详情</el-button>
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['system:nursing:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['system:nursing:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <!-- 护理项目详情抽屉 -->
    <nursing-view-drawer ref="nursingViewRef" />
    <!-- 添加或修改护理项目对话框 -->
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="nursingRef" :model="form" :rules="rules" label-width="100px">
        <el-row>
          <el-col :span="24">
            <el-form-item label="名称" prop="name">
...
const nursingList = ref<NursingProject[]>([])
...
function getList() {
  loading.value = true
  listNursing(queryParams.value).then(response => {
    nursingList.value = response.rows
...
/** 提交按钮 */
function submitForm() {
  proxy.$refs["nursingRef"].validate((valid: boolean) => {
    if (valid) {
      if (form.value.id != null) {
        updateNursing(form.value).then(() => {
          proxy.$modal.msgSuccess("修改成功")
          open.value = false
          getList()
        })
      } else {
        addNursing(form.value).then(() => {
...
/** 详情按钮操作 */
function handleViewData(row: NursingProject) {
  proxy.$refs["nursingViewRef"].open(row.id)
}

/** 导出按钮操作 */
function handleExport() {
  proxy.download('system/nursing/export', {
    ...queryParams.value
